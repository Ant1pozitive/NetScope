...
This collector summarizes gradient-like payloads in an observation-only way.
...
class GradientCollector(BaseCollector):
    """
    Collect structured gradient summaries.
    """

    collector_kind = CollectorKind.GRADIENT
    ...
    def collect(
        ...
        *,
        ...
        """
        Collect gradient summaries from a target payload.
        """
        ...
        for index, (path, value) in enumerate(entries):
            ...
            if is_tensor:
                if summary.get("finite_fraction") == 1.0:
                    finite_tensor_count += 1
                if summary.get("l2_norm") == 0.0:
                    zero_tensor_count += 1

            record = self._new_record(
                target=target_obj,
                value=summary,
                value_type="tensor_gradient" if is_tensor else "gradient_leaf",
                metadata={
                    "path": path,
                    "index": index,
                    "is_tensor": is_tensor,
        ...
        batch = self._new_batch(
            ...
            metadata={
                ...
                "finite_tensor_count": finite_tensor_count,
    ...
    def _summarize_tensor(self, tensor: Any) -> dict[str, Any]:
        ...
        if finite_values.numel() == 0:
            ...
            positive_fraction = float((finite_values > 0).float().mean().item())
            negative_fraction = float((finite_values < 0).float().mean().item())
    ...
    def _safe_context(context: Any) -> Any:
        ...
        if isinstance(context, Mapping):
            return {
                str(key): GradientCollector._safe_context(value)
                for key, value in context.items()
            }

        if isinstance(context, Sequence) and not isinstance(context, (str, bytes, bytearray)):
            return [GradientCollector._safe_context(item) for item in context]
